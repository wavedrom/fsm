...
const stateEnums = desc => {
  ...
  return [
    ...
    ...(desc.states).map((state, idx) =>
      'wire [' + (desc.width - 1) + ':0] ' + desc.name + '_' + name(state).padEnd(maxW) + ' = ' + idx + ';'
    ),
    ''
  ];
};
...
const stateConds = desc => {
  ...
  return [
    ...
    ...(desc.states).flatMap(state =>
      state.next.map(next =>
        'wire ' + desc.name + '_' + name(state, next).padEnd(maxSN + 1) +
        ' = ' +
        '(' +
          '(' + desc.name + '_state == ' + desc.name + '_' + name(state).padEnd(maxS) + ')' +
          ' & ' +
          '(' +
          (desc.cond
            ? (next.cond + ' == ' + desc.cond)
...
const selectNext = desc => {
  ...
  return [
    ...
    ...(desc.states).flatMap(state =>
      state.next.map(next =>
        '    ' + desc.name + '_' + name(state, next).padEnd(maxSN + 2) + ': ' +
...
const flipFlopActions = desc => {
  ...
  desc.states.map(state => {
    state.next.map(next => {
      if (next.act) {
        trans.push(
          '      ' + name(desc, state, next) + ' : begin',
          ...toArray(next.act).map(e => '        ' + e + ';'),
          '      end'
        );
      }
    });
  ...
  desc.states.map(state => {
    if (state.onExit) {
      exits.push(
        '      ' + name(desc, state, 'onExit') + ' : begin',
        ...toArray(state.onExit).map(e => '        ' + e + ';'),
        '      end'
      );
    }
  });

  if (trans.length === 0 && exits.length === 0) {
    return [];
  }
...
const alwaysAscii = desc => {
  ...
  const maxS = desc.states.reduce(maxName, 2);
  const maxLen = desc.states.reduce((res, state) => Math.max(res, name(state).length), 1);
  return [
    'reg [' + (maxLen * 8 - 1) + ':0] ' + desc.name + '_state_ascii;',
    'always @(*)',
    '  case ({' + desc.name + '_state})',
    ...(desc.states).map(state =>
      '    ' + desc.name + '_' + name(state).padEnd(maxS) + ' : ' + desc.name + '_state_ascii = "' + name(state).padEnd(maxLen) + '";'
    ),
