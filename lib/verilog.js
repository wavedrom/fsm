...
const normDesc = desc => {
  ...
  if (!Array.isArray(regs) && typeof regs === 'object') {
    regs = Object.keys(regs).map(name => {
      const val = regs[name];
      const res = {name};
      if (typeof val === 'object') {
        Object.assign(res, val);
...
const enterConditions = desc => {
  ...
  res.push(...Object.keys(o).map(to =>
    'wire ' + desc.name + '_' + pad(maxS + 8)(to + '_onEnter') + ' = (' +
...
const flipFlopActions = desc => {
  ...
  desc.states.map(state => {
    state.next.map(next => {
      cases(trans, next.act, name(desc, state, next));
    });
  });
  ...
  const enters = [];
  desc.states.map(state => {
    cases(enters, state.onEnter, name(desc, state, 'onEnter'));
  });

  if ([trans, enters, exits].reduce((res, e) => res + e.length, 0) === 0) {
    return [];
  }
